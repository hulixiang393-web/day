...
export function scheduleReminder(task) {
  const targetDate = task.endDate || task.startDate;
  if (!targetDate || !task.reminders || task.reminders.length === 0) return [];

  const timers = [];

  for (const reminder of task.reminders) {
    const dueDateTime = new Date(targetDate);
    const timeStr = task.endTime || task.startTime;
    if (timeStr) {
    ...
    const timer = setTimeout(() => {
      sendNotification(`⏰ ${task.title}`, {
        body: `将在 ${reminder.minutes} 分钟后到期${timeStr ? ` (${timeStr})` : ''}`,
