...
export const TaskModel = {
  ...
  async getByDate(date) {
    const all = await db.getAll(STORE);
    return all.filter((t) => t.endDate === date || t.startDate === date);
  },
  ...
  async create(data) {
    const task = {
      ...
      endDate: data.endDate || data.dueDate || null,
      endTime: data.endTime || data.dueTime || null,
  ...
  async getStats(startDate, endDate) {
    ...
    const filtered = all.filter((t) => {
      ...
      return t.endDate >= startDate && t.endDate <= endDate;
    });
